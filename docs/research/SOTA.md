...
- **EXPERIMENTAL RESULT — H1 is PARTIALLY SUPPORTED, and mostly rejected at this scale.** Three distinct measurements, and the gap between them is the finding.
  ...
  - *The scaling signal:* per-iteration cost is nearly flat across a 20x range of nonzeros — **57 us at 7,777 nnz, 111 us at 144,848** — the signature of latency-bound execution. It is why the small set loses: the device is mostly idle.
  - **TESTED AT SCALE, and the hypothesis holds.** An earlier revision recorded the above as an inference and stated that *no model in this repository is large enough to test it*. **That was wrong.** 21 models were being skipped by `validate_netlib` — not for size, but because `data/netlib_readme.txt` carries no reference objective for them. They are the Kennington families and the QAP relaxations, and they dwarf the validated set: `ken-18` has 105,127 rows and `osa-60` has 1,397,793 nonzeros. `benchmarks/validate_crossmethod.cpp` solves them with both methods and checks the objectives against each other (60 s budget per method): **21/21 solved, 0 disagreements**. Where both methods succeeded (16 instances) the first-order path totals **44.53 s against the simplex's 106.56 s — 2.39x faster** — with `ken-11` at **21.0x** (21.67 s vs 1.03 s), `pds-10` at **15.3x**, and `osa-60` at **7.5x**. Five more were solved only by the first-order path within the simplex's budget, including `ken-18` in **12.63 s**. So H1's wall-clock claim is **supported at scale on real industrial-structured models**, not by extrapolation. Two limits stay on that: the five "first-order only" results are relative to a 60 s simplex budget rather than a simplex run to completion, and cross-method agreement is weaker evidence than a published optimum. Layer D of the benchmark plan (a refinery-style generator) still does not exist, so no refinery-*specific* claim is admissible.
  ...
  - The lesson about H1's wording holds: H1 asks whether GPU sparse linear algebra can accelerate *repeated LP operations*, and the assumption travelling silently alongside it — that the acceleration would appear inside the simplex — is false. The defensible claim is that **one algorithm change removes the synchronization barrier completely, and that this buys a decisive wall-clock win on large models (2.39x aggregate on Kennington, up to 21x per instance) while losing on small ones (4.4x slower under 2,500 rows).** Size, not degeneracy, is what separates the two regimes.
  - **A methodological lesson worth more than the result.** The models that proved H1 had been in this repository the whole time. They were invisible because the validator skipped anything it could not score against a published optimum, and it skipped them *before solving*, so they never appeared in a failure list — only in a "no published reference" count that read as bookkeeping. **An aggregate computed only over instances that are easy to score will flatter the engine, and the exclusions will not look like exclusions.** Coverage gaps must be enumerated by name, not counted.
