...
SIHPS_TEST(milp_proves_integer_infeasibility_by_exhausting_nodes) {
    ...
    SIHPS_ASSERT_EQ(result.integer_propagation_prunes, 1);
}
...
SIHPS_TEST(milp_integer_inequality_rounding_tightens_lattice_rhs) {
    LpProblem lp;
    lp.A = CSRMatrix::from_triplets(1, 1, {Triplet{0, 0, 2.0}});
    lp.obj = {-1.0};
    lp.rhs = {3.0};
    ...
    SIHPS_ASSERT_TRUE(result.integer_rhs_tightenings >= 1);
    SIHPS_ASSERT_EQ(result.nodes_processed, 1);
}

SIHPS_TEST(milp_integer_inequality_rounding_handles_negative_g_rows) {
    LpProblem lp;
    lp.A = CSRMatrix::from_triplets(1, 1, {Triplet{0, 0, -2.0}});
    lp.obj = {-1.0};
    lp.rhs = {-3.0};
...
SIHPS_TEST(milp_optional_gmi_cut_preserves_g_row_direction) {
    MilpSolverOptions options;
    options.use_rounding_heuristic = false;
    options.enable_root_cover_cuts = false;
    options.enable_root_integer_rounding_cuts = false;
...
SIHPS_TEST(milp_integer_gcd_gate_rejects_interval_feasible_parity_gap) {
    LpProblem lp;
    lp.A = CSRMatrix::from_triplets(
        1, 3, {Triplet{0, 0, 2.0}, Triplet{0, 1, 2.0}, Triplet{0, 2, 2.0}});
    lp.obj = {0.0, 0.0, 0.0};
    lp.rhs = {3.0};
    lp.row_types = {'E'};
    lp.lower = {0.0, 0.0, 0.0};
    lp.upper = {10.0, 10.0, 10.0};
    sihps::apply_default_row_bounds(lp);
    MilpSolverOptions options;
    options.use_rounding_heuristic = false;
    options.enable_root_cover_cuts = false;
