...
int main(void)
{
    ...
    model.A.x[0] = 1.0; model.A.x[1] = 1.0;
    model.c[0] = 1.0; model.c[1] = 1.0;
    model.rlow[0] = 1.5; model.rupp[0] = SK_INFINITY;
    ...
    if (sk_solve(&model, &options, &solution) != SK_OK || solution.result != SK_RESULT_GAP_LIMIT ||
        !solution.x || fabs(solution.x[0] - 1.0) > 1e-5 || fabs(solution.x[1] - 1.0) > 1e-5 ||
