...
int main(void)
{
    ...
    m.Q->p = (int *)calloc(3, sizeof(int)); m.Q->i = (int *)calloc(4, sizeof(int));
    ...
    if (sk_solve(&m, &o, &s) != SK_OK || s.result != SK_RESULT_OPTIMAL || s.iterations != 0 ||
        fabs(s.x[0] - 2.0 / 3.0) > 1e-10 || fabs(s.x[1] - 5.0 / 3.0) > 1e-10 ||
        fabs(s.objective + 13.0 / 3.0) > 1e-10 || sk_verify(&m, &s) != SK_OK ||
