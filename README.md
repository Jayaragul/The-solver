...
| CUDA QPCBLEND convergence extension | 1,000,000 iterations reduce KKT residual from `6.528e-05` to `2.275e-05`; `--check-every 1000` cuts runtime about 9%, independent check passes | recorded |
| CUDA QP device-provenance refresh | QAFIRO and DPKLO1 independently certified on RTX 3050; QPCBLEND remains an explicit iteration limit with dual residual `5.513e+00` | recorded |
...
| MILP relaxation time-budget propagation | Remaining wall budget reaches cold/warm LP solves and probes; p0201 certifies in 1.055 s under a 10-second cap | passed |
...
3. The source-level comparison with the referenced LP-and-MLIP-Solver project,
   including adopted and rejected techniques, is recorded in
   `docs/research/LP_MLIP_SOLVER_COMPARISON.md`.
